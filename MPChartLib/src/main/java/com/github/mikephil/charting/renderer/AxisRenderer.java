...
public abstract class AxisRenderer extends Renderer {
	...
	protected void computeAxisValues(float min, float max) {
		...
		double interval = calculateInterval(range, labelCount);

		int calculatedLabelsCount = mAxis.isCenterAxisLabelsEnabled() ? 1 : 0;

		Pair<Double, Integer> result = calculateLabelsCountAndInterval(calculatedLabelsCount, interval, yMin, yMax, range, labelCount);
		interval = result.first;
		calculatedLabelsCount = result.second;
		...
		if (mAxis.isCenterAxisLabelsEnabled()) {
			...
			if (mAxis.mCenteredEntries.length < calculatedLabelsCount) {
				mAxis.mCenteredEntries = new float[calculatedLabelsCount];
			}

			float offset = (float) interval / 2f;

			for (int i = 0; i < calculatedLabelsCount; i++) {
				mAxis.mCenteredEntries[i] = mAxis.mEntries[i] + offset;
			}
		}
	}

	protected Pair<Double, Integer> calculateLabelsCountAndInterval(int newLabelsCount, double interval, float yMin, float yMax, double range, int labelCount) {
		Pair<Double, Integer> result;
		// force label count
		if (mAxis.isForceLabelsEnabled()) {
			result = calculateForcedLabelCount(range, labelCount, yMin);
			// no forced count
		} else {
			result = calculateNoForcedLabelCount(interval, newLabelsCount, yMin, yMax);
		}
		return result;
	}
	...
	protected Pair<Double, Integer> calculateForcedLabelCount(double range, int labelCount, float min) {
		...
		return new Pair<>(interval, labelCount);
	}
